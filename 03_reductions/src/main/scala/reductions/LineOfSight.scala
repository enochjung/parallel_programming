...
object LineOfSight extends LineOfSightInterface:
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree =
    if (end - from <= threshold) {
      val maxTan = upsweepSequential(input, from, end)
      Tree.Leaf(from, end, maxTan)
    } else {
      val mid = (from + end) / 2
      val (tL, tR) =
        parallel(
  ...
  def downsweep(
    ...
    tree match {
      ...
      case Tree.Leaf(from: Int, until: Int, maxPrevious: Float) =>
