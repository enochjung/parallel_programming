...
class KMeans extends KMeansInterface:
  ...
  def classify(
    ...
    if points.isEmpty then ParMap()
    else if means.isEmpty then ParMap()
  ...
  def converged(
    ...
    val zipped = oldMeans zip newMeans
    val distSum = zipped.foldLeft(0.0)((x, t) => x + t(0).squareDistance(t(1)))
    distSum <= eta
